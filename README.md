...
洛克王国世界完整数据爬虫 + 战斗模拟器 + MCTS AI 对战引擎 + **PVP 自动战斗脚本**。

爬取 BWIKI 精灵图鉴，附带终端图鉴查看器、自由组队战斗模拟器，以及基于 **蒙特卡洛树搜索（MCTS）** 的 AI 智能体，支持通过 YAML 配置文件指导 AI 作战风格。v1.1 新增 **游戏客户端自动化模块**，可直接驱动游戏进行 PVP 自动战斗。
...
| 🎮 PVP 自动挑战 | 直接驱动游戏客户端，自动执行技能、检测死亡、换宠逃跑（v1.1） |
